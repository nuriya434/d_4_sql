...
SELECT * FROM orders WHERE LEFT(ship_country, 1) = 'U';
...
SELECT COUNT(DISTINCT customer_id) FROM customers WHERE region IS NOT NULL;
...
FROM (
  ...
  EXCEPT
  SELECT country FROM employees
) AS countries_without_employees;
